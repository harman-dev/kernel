...
/* Hold time change to increase delay between SDA and SCL when SoC at -40
 * deg. C */
#define DW_IC_HOLD_TIME 100
...
int i2c_dw_init(struct dw_i2c_dev *dev)
{
	...
	if (dev->adapter.nr == 2)       // I2C port ipod
	{
		hcnt=hcnt*8;
		lcnt=lcnt*8;
		printk(KERN_WARNING "%s: Warning, overwrite fast-mode settings Fast-mode HCNT:LCNT = %d:%d  %i\n", __FUNCTION__, hcnt, lcnt,dev->adapter.nr );
	}
	...
	if (dev->sda_hold_time) {
		dev->sda_hold_time = DW_IC_HOLD_TIME; 
...
i2c_dw_xfer_msg(struct dw_i2c_dev *dev)
{
	...
	/*
	 * If i2c_msg index search is completed, we don't need
	 * TX_EMPTY interrupt any more.  Disable TX_EMPTY interrupt if
	 * we are waiting for a RX_FULL event.
	 */
	if (dev->msg_write_idx == dev->msgs_num ||
	    dev->rx_outstanding >= dev->rx_fifo_depth)
