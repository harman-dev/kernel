...
static const struct mfd_cell intel_lpss_tlin_cell = {
	.name = "tlin",
	.num_resources = ARRAY_SIZE(intel_lpss_dev_resources),
	.resources = intel_lpss_dev_resources,
};
...
{
	/* devIdle infrastructure may have already set this up. */
	if (lpss->dev->power.set_latency_tolerance != NULL)
		return;
...
static int intel_lpss_assign_devs(struct intel_lpss *lpss)
{
	...
	case LPSS_DEV_I2C:
		...
		break;
	case LPSS_DEV_UART:
		/*0x2FC UART_Capabilities reg - value 3 is UART3*/
		if ((lpss->caps & 0x03) == 0x03) {
			cell = &intel_lpss_tlin_cell;
		} else {
