...
static void intel_pstate_get_min_max(struct cpudata *cpu, int *min, int *max)
{
	...
	*max = clamp_t(int, max_perf_adj,
			cpu->pstate.min_pstate + cpu->floor_min,
		cpu->pstate.turbo_pstate);

	min_perf = fp_toint(max_perf * limits->min_perf);
	*min = clamp_t(int, min_perf, cpu->pstate.min_pstate + cpu->floor_min,
		max_perf);
}
...
static inline int32_t get_avg_freq(struct cpudata *cpu)
{
	u64 avg_freq;

	avg_freq = div64_u64(cpu->sample.aperf * cpu->pstate.max_pstate *
		cpu->pstate.scaling, cpu->sample.tsc);

	return (int32_t)avg_freq;
}
