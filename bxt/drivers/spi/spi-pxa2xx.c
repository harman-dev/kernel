...
static void pump_transfers(unsigned long data)
{
	...
	/* saturn_flash was failing when we reload dynamic_spi_node mode=0
	 * caould not find the exact root cause, this is  WA to make it work
	 * This delay is applicable only to device /dev/spidev1.1 */
	if (spi->max_speed_hz == 50000000 && spi->master->bus_num == 1
			&& spi->chip_select == 1) {
		mdelay(10);
	}
}
