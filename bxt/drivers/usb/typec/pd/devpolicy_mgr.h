...
static inline int devpolicy_set_vbus_state(struct devpolicy_mgr *dpm,
						bool state)
{
	if (dpm && dpm->phy && dpm->phy->enable_vbus)
		return dpm->phy->enable_vbus(dpm->phy, state);
...
static inline int devpolicy_set_hpd_state(struct devpolicy_mgr *dpm,
						bool state)
{
	if (dpm && dpm->phy && dpm->phy->set_hpd_state)
		return dpm->phy->set_hpd_state(dpm->phy, state);
