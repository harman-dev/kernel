...
#define IS_PMC_PEBS_ENABLED(ia32_perf_global_ctrl_reg_val, ia32_pebs_enable_reg_val) \
    ...
    FOR_EACH_REG_ENTRY(pecb, i) {
        ...
        if (DRV_CONFIG_pebs_mode(pcfg)
            ...
            PEBS_Reset_Counter(this_cpu,
                               (ECB_entries_reg_id(pecb,i) - IA32_PMC0),
                               ECB_entries_reg_value(pecb,i));
        }
...
)
{
    ...
    if (GLOBAL_STATE_current_phase(driver_state) == DRV_STATE_RUNNING) {
        ...
        if (multi_pebs_enabled) {
            global_control_val = SYS_Read_MSR(IA32_PERF_GLOBAL_CTRL);
            pebs_enable_val    = SYS_Read_MSR(IA32_PEBS_ENABLE);

            if ( IS_FIXED_CTR_ENABLED(global_control_val)
                || !IS_PMC_PEBS_ENABLED(global_control_val, pebs_enable_val)) {
                SEP_PRINT_DEBUG("global_control_val = 0x%llx pebs_enable_val = 0x%llx\n",
                                global_control_val, pebs_enable_val);
                SYS_Write_MSR(IA32_DEBUG_CTRL, DISABLE_FRZ_ON_PMI(ECB_entries_reg_value(pecb,3)));
            }
        }
    }
...
)
{
    ...
    SEP_PRINT_DEBUG("PMU control_data 0x%p, buffer 0x%p\n", PMU_register_data, buffer);
...
)
{
    ...
    if (DRV_CONFIG_pebs_mode(pcfg)
        && (DRV_CONFIG_pebs_record_num(pcfg) == 1)) {
...
)
{
    ...
    if (DRV_CONFIG_pebs_record_num(pcfg)) {
        FOR_EACH_DATA_REG(pecb, i) {
            if (ECB_entries_precise_get(pecb, i) == 1) {
                PEBS_Reset_Counter(this_cpu, (ECB_entries_reg_id(pecb,i) - IA32_PMC0),
                    ECB_entries_reg_value(pecb,i));
            }
        } END_FOR_EACH_DATA_REG;
    }
...
)
{
    ...
    if (DRV_CONFIG_ds_area_available(pcfg) && DRV_CONFIG_pebs_mode(pcfg)) {
        SYS_Write_MSR(IA32_PEBS_ENABLE, 0LL);
    }
    ...
    SEP_PRINT_DEBUG("    msr_val(IA32_FIXED_CTRL)=0x%llx \n", CPU_STATE_pmu_state(pcpu)[2]);
...
)
{
    ...
    for (i = 0; i < LBR_num_entries(lbr); i++) {
        ...
        if (DRV_CONFIG_collect_callstacks(pcfg)) {
...
#define IA32_MSR_PLATFORM_INFO 0xCE
    ...
    SEP_PRINT_DEBUG ("perfver4_Platform_Info: Read from MSR_ENERGY_MULTIPLIER reg is %llu\n", SYS_Read_MSR(MSR_ENERGY_MULTIPLIER));
