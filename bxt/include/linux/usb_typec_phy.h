...
#define TYPEC_NOMINAL_VOLTAGE		5000000	/* uV */
...
struct typec_pd_profile {
	int nom_voltage;	/* Nominal Voltage */
...
struct typec_phy {
	...
	int (*enable_vbus)(struct typec_phy *phy, bool en);
	bool (*is_vbus_on)(struct typec_phy *phy);
	int (*enable_vconn)(struct typec_phy *phy, bool en);
	int (*supply_vconn)(struct typec_phy *phy, bool en);
	bool (*is_vconn_on)(struct typec_phy *phy);
	int (*set_hpd_state)(struct typec_phy *phy, bool state);
};

/* TODO: Clean these additional declarations */
...
{
	if (phy && phy->is_vconn_on)
		return phy->is_vconn_on(phy);
...
{
	if (phy && phy->enable_vconn) {
		if (phy && phy->supply_vconn)
			phy->supply_vconn(phy, en);
		return phy->enable_vconn(phy, en);
	}
...
extern struct typec_phy *typec_get_phy(int type);
extern int typec_get_cc_orientation(struct typec_phy *x);
extern int typec_get_pd_profile(struct typec_pd_profile *profile);
#else
struct typec_phy *typec_get_phy(int type) { return NULL; }
int typec_get_cc_orientation(struct typec_phy *x) { return -ENODEV; }
int typec_get_pd_profile(struct typec_pd_profile *profile) { return -ENODEV; }
#endif
