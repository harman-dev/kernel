...
static const struct pci_device_id ish_pci_tbl[] = {
	...
	{PCI_DEVICE(PCI_VENDOR_ID_INTEL, BXT_M_AX_DEVICE_ID)},
	{PCI_DEVICE(PCI_VENDOR_ID_INTEL, BXT_M_BX_DEVICE_ID)},
	{PCI_DEVICE(PCI_VENDOR_ID_INTEL, BXT_P_AX_DEVICE_ID)},
	{0, }
};
...
static int ish_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	ISH_INFO_PRINT(KERN_ALERT
		"[heci-ish]: %s():+++ [Build "BUILD_ID "]\n",
		__func__);
	ISH_INFO_PRINT(KERN_ALERT
		"[heci-ish] %s() running on %s revision [%02X]\n", __func__,
		pdev->device == BXT_M_AX_DEVICE_ID ? "BXT Ax" :
		pdev->device == BXT_M_BX_DEVICE_ID ? "BXT Bx" :
		pdev->device == BXT_P_AX_DEVICE_ID ? "BXTP Ax" :
	...
	__func__);
#elif defined(SUPPORT_BXT_ONLY)
	dev->pdev->device = BXT_M_AX_DEVICE_ID;
	ISH_DBG_PRINT(KERN_ALERT "[heci-ish] %s() device forced to BXT\n",
	 __func__);
#endif
	mutex_lock(&heci_mutex);
	if (heci_pci_device) {
