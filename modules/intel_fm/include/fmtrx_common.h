...
#define KHZ_TO_62_5_KHZ(X) (X * (1 << 4) / 1000)
...
enum fmtrx_v4l2_cid_private {
	...
	V4L2_CID_PRIV_INTEL_FMRX_GET_DEBUG_INFO,
...
struct ch_status_info {
	u16 tune_status;
	u16 search_status;
	u16 scan_status;
};

struct scan_buf_info {
	u16 buf_start;
	u16 buf_end;
	u16 write_ptr;
	u16 host_read_ptr;
...
struct debug_info {
	...
	struct channel_dbg_info ch_info;
	struct scan_buf_info scan_info;
	struct rds_buf_info rds_info;
};

struct fmrx_platform {
	struct device dev;
	struct completion event_wait;
	struct completion read_cmd_wait;
	struct completion write_cmd_wait;
	struct completion power_cmd_wait;
	struct completion audio_cmd_wait;
	struct completion irq_cmd_wait;
	struct completion msg_process_wait;
	struct completion sock_process_wait;
	struct mutex cmd_sync_lock;
	void *p_hci_evt;
	enum interrupt_vector fmr_vector_type;
	const struct firmware *fw_entry;
#ifdef FMR_INTR_MODE
	struct task_struct *msg_process_task;
	bool msg_process_task_stop;
#endif
#ifdef CONFIG_IUI_FM_FMR
	bool fmr_fm_msg_process_task_stop;
	struct task_struct *fmr_fm_msg_process_task;
#endif
#ifdef LD_DRIVER
	struct fm_ld_drv_register ld_driver;
#endif
};
 ...
 * @data Pointer to debug information
 */
