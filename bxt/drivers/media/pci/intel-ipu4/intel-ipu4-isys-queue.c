...
static int intel_ipu_isys_queue_dump_frame_counter(
	 struct intel_ipu4_isys_video *av)
{
	struct intel_ipu4_isys_queue *aq = &av->aq;
	struct intel_ipu_isys_frame_counter *kfc, *kfc0;
	unsigned long flags;

	dev_dbg(&av->isys->adev->dev, "dump fc %s:\n", av->vdev.name);

	spin_lock_irqsave(&aq->lock, flags);
	list_for_each_entry_safe(kfc, kfc0, &aq->fc, head)
		dev_dbg(&av->isys->adev->dev,
			"buf %d with fc %d\n",
			kfc->index, kfc->frame_counter);
	spin_unlock_irqrestore(&aq->lock, flags);

	return 0;
}

static bool __match_and_release_frame_counter
	(struct intel_ipu4_isys_buffer *ib, uint32_t *frame_counter)
{
	...
	list_for_each_entry_safe(kfc, kfc0, &aq->fc, head) {
		if (kfc->index == index) {
			*frame_counter = kfc->frame_counter;
			is_match = true;
			list_del(&kfc->head);
			break;
		}
	...
	struct intel_ipu4_isys_request *ireq)
{
	...
	list_for_each_entry(ib, &ireq->buffers, req_head) {
		...
		is_match =
			__match_and_release_frame_counter(ib, &frame_counter);
		if (!is_match)
			goto out;

		if (frame_counter_pre < 0) {
			frame_counter_pre = frame_counter;
			continue;
		}
		if (frame_counter_pre != frame_counter)
			goto out;
	}
	spin_unlock_irqrestore(&ireq->lock, flags);
	ireq->frame_counter = frame_counter;
	return 0;
	 ...
	 uint32_t frame_counter, uint32_t index)
{
	...
	list_for_each_entry_safe(kfc, kfc0, &aq->fc, head) {
		if (kfc->index == index) {
			dev_err(&av->isys->adev->dev,
				"error: buf %d with fc %d exist already\n",
				kfc->index, kfc->frame_counter);
			spin_unlock_irqrestore(&aq->lock, flags);
			/*
			 * TODO: maybe need to update new fc with existing
			 * index instead of return error in future
			 */
			return -EINVAL;
		}
	}
	...
	kfc = kmalloc(sizeof(*kfc), GFP_KERNEL);
	if (!kfc)
		return -ENOMEM;
	kfc->frame_counter = frame_counter;
	kfc->index = index;
	spin_lock_irqsave(&aq->lock, flags);
	list_add_tail(&kfc->head, &aq->fc);
	spin_unlock_irqrestore(&aq->lock, flags);
	intel_ipu_isys_queue_dump_frame_counter(av);
			...
			struct intel_ipu4_isys_buffer_list *bl)
{
	...
	list_for_each_entry(ib, &bl->head, head) {
		...
		aq = vb2_queue_to_intel_ipu4_isys_queue(vb->vb2_queue);
		av = intel_ipu4_isys_queue_to_video(aq);
		...
		if (av->link_id) {
			is_match = __match_and_release_frame_counter(ib,
					&frame_counter);
			if (!is_match) {
				ret = -EINVAL;
				dev_err(&ip->isys->adev->dev,
					"error: %s buffer could not match fc\n",
					av->vdev.name);
				goto error;
			}
			...
			if (frame_counter_pre != frame_counter) {
				ret = -EINVAL;
				dev_err(&ip->isys->adev->dev,
				 "pre's not equal to current %ld, %d\n",
				 frame_counter_pre, frame_counter);
				goto error;
			}
		}
	}
	bl->frame_counter = frame_counter;
					...
					bool error)
{
	...
	while ((ireq = intel_ipu4_isys_next_queued_request(ip))) {
		...
		rval = intel_ipu4_isys_req_prepare(mdev, ireq, ip, set);
		if (rval) {
			mutex_unlock(&pipe_av->isys->stream_mutex);
			goto out_requeue;
		}
	...
	do {
		...
		fw_dump_isys_frame_buff_set(&pipe_av->isys->adev->dev, buf,
			ip->nr_output_pins);
			   ...
			   enum vb2_buffer_state state)
{
	...
	spin_lock_irqsave(&aq->lock, flags);
	while (!list_empty(&aq->fc)) {
		struct intel_ipu_isys_frame_counter *kfc =
			list_first_entry(&aq->fc,
				struct intel_ipu_isys_frame_counter, head);
			list_del(&kfc->head);
			spin_unlock_irqrestore(&aq->lock, flags);
			kfree(kfc);
			spin_lock_irqsave(&aq->lock, flags);
					...
					struct ipu_fw_isys_resp_info_abi *info)
{
	...
	if (ip->has_sof) {
		...
		ns = ktime_to_ns((wall_clock_ts_on) ? ktime_get_real() : ktime_get());
				     ...
				     struct ipu_fw_isys_resp_info_abi *info)
{
	...
	dev_dbg(&isys->adev->dev, "buffer: %s: received buffer %8.8x %d %d\n",
		intel_ipu4_isys_queue_to_video(aq)->vdev.name, info->pin.addr,
		info->frame_counter, info->written_direct);
				 ...
				 struct ipu_fw_isys_frame_buff_set_abi *set)
{
	...
	rval = intel_ipu_isys_req_match_frame_counter(ireq);
	if (rval)
		return rval;
