...
MODULE_AUTHOR("Copyright (C) 2009-2017 Intel Corporation");
...
)
{
    ...
    if ( (pax_status.pid == current->pid) ||
        ...
        if (nmi_watchdog_restore != '0') {
            kthread_run(&pax_Enable_NMIWatchdog, NULL, "pax_enable_nmi_watchdog");
...
#if !defined (DRV_ANDROID) && !defined(DRV_CHROMEOS) && defined(CONFIG_HARDLOCKUP_DETECTOR) && LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)
        kthread_run(&pax_Disable_NMIWatchdog, NULL, "pax_disable_nmi_watchdog");
        msleep(20);
#endif
