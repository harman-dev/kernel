...
static long intel_ipu_isys_vidioc_private(struct file *file, void *fh,
			bool valid_prio, unsigned int cmd, void *arg)
{
	struct intel_ipu4_isys_video *av = video_drvdata(file);
	struct intel_ipu4_isys_pipeline *ip = &av->ip;
	int ret = 0;

	switch (cmd) {
	case VIDIOC_INTEL_IPU_SET_LINK_ID: {
		uint8_t  link_id = *(uint8_t *)arg;

		if (ip->streaming) {
			dev_err(&av->isys->adev->dev, "ERROR in ioctl: isys is streaming\n");
			return -EBUSY;
		}

		dev_dbg(&av->isys->adev->dev, "set link_id %d\n", link_id);

		if ((is_intel_ipu4_hw_bxt_b0(av->isys->adev->isp)
			&& link_id != 0) || link_id >= N_IPU_FW_ISYS_LINK_ID)
			return -EINVAL;

		av->link_id = link_id;
		break;
	}
	case VIDIOC_INTEL_IPU_SET_FRAME_COUNTER: {
		struct intel_ipu_frame_counter *fc =
			(struct intel_ipu_frame_counter *)arg;

		if (!av->link_id)
			return 0;
		dev_dbg(&av->isys->adev->dev, "set frame_counter %d\n",
				fc->frame_counter);
		ret = intel_ipu_isys_queue_prepare_frame_counter(av,
			fc->frame_counter, fc->index);
		break;
	}
	default:
				 ...
				 struct intel_ipu4_isys_buffer_list *bl)
{
	...
	if (bl) {
		...
		rval = intel_ipu4_isys_req_prepare(&av->isys->media_dev,
					    ireq, ip, buf);
		if (rval)
			goto out_put_stream_opened;
	}
...
#ifdef CONFIG_COMPAT
static long intel_ipu_isys_compat_ioctl(struct file *file, unsigned int cmd,
					unsigned long arg)
{
...
static const struct v4l2_ioctl_ops ioctl_ops_splane = {
	...
	.vidioc_default = intel_ipu_isys_vidioc_private,
};
...
#ifdef CONFIG_COMPAT
	.compat_ioctl32 = intel_ipu_isys_compat_ioctl,
#endif
	.mmap = vb2_fop_mmap,
	.open = video_open,
