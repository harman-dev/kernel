...
static int fmrx_hw_get_rds_count_status(
		struct fmrx_platform *fmrx_plat,
		struct rds_buf_info *buf_info,
		u16 *group_count,
		u16 *discard_count);

/* Get channel status
 * @status_info Pointer to the Channel status information
 */
static int fmrx_hw_get_channel_status_info(
		struct fmrx_platform *fmrx_plat,
		struct ch_status_info *status_info);

/* Get Scan buffer info
 * @buf_info Pointer to the scan buffer information
*/
static int fmrx_hw_get_scan_buf_info(
		struct fmrx_platform *fmrx_plat,
		struct scan_buf_info *buf_info);
	...
	struct fmrx_platform *fmrx_plat)
{
	...
	/* Send command to DSP */
	err = fmtrx_send_cmd(
			fmrx_plat, FMTRX_RX, &cmd_pkt, 0);
	if (0 != err) {
		fmtrx_sys_log
		(CRIT, "%s: %s %d,Send RF power off command failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
	...
	struct fmrx_platform *fmrx_plat)
{
	...
	/* Set PC to zero */
	err = fmtrx_write_field(fmrx_plat,
			MINIDSPCTL_ADDR, MINIDSPCTL_PC_POS32,
			MINIDSPCTL_PC_WIDTH, 0);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Write PC failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}

	/* Set run bit to start executing FW */
	err = fmtrx_write_bit(fmrx_plat,
			MDSPHALTCTL_ADDR,
			MDSPHALTCTL_MST_RUN_POS32, 1);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Write RUN bit failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
		...
		bool *status)
{
	...
	err = fmtrx_sys_reg_read32(fmrx_plat,
			MINIDSPCTL_ADDR, &val);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Read 32-bit failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
		...
		struct fmrx_platform *fmrx_plat)
{
	...
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Write HALT bit failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
		...
		s16 rssi_other_offset)
{
	...
	err = fmtrx_sys_reg_write16(fmrx_plat,
			FMR_RXMAIN_RSSI_OTHER_OFFS_ADDR,
			rssi_other_offset);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Write 16-bit failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
		...
		enum deemphasis_type type)
{
	...
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Set De-emp failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmrx_hw_set_audio_deemp_exit;
	}
		...
		bool mute_wait)
{
	...
	if ((true == mute_wait) &&
			(true == mute_enable)) {
		do {
			err = fmtrx_sys_reg_read16(fmrx_plat,
				FMR_RXMAIN_AUDIO_GAIN_OUT_L_ADDR,
				&left);
			if (0 != err) {
				fmtrx_sys_log
				(CRIT, "%s: %s %d,Read 16-bit failed! %d\n",
					FILE, __func__,
					__LINE__, err);
				goto fmrx_hw_set_mute_exit;
			}
		...
		struct agc *cfg)
{
	...
	if (0 != err) {
		fmtrx_sys_log
		(CRIT, "%s: %s %d,Send AGC gain control command failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
		...
		u16 bad_blks_search)
{
	...
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Send RDS Cfg command failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		...
		u32 interrupt_clr)
{
	...
	err = fmtrx_sys_reg_write32(fmrx_plat,
				INTSETCTL_ADDR,
				interrupt_clr << 16);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Write 32-bit failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
		...
		u32 clk_switch_range_104)
{
	...
	err = fmrx_hw_set_clk_source(fmrx_plat, CLK_SRC_MAINCLK);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Switch to main clock failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmrx_hw_channel_tune_exit;
	}
	...
	if (0 != err) {
		fmtrx_sys_log
		(CRIT, "%s: %s %d,Send Channel tune command failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmrx_hw_channel_tune_exit;
	}
	...
	if ((frequency >
	    ...
	    (frequency <
	    (FREQUENCY_104MHZ + clk_switch_range_104))) {
		err = fmrx_hw_set_clk_source(fmrx_plat,
...
#ifdef CONFIG_IUI_FM_FMR
	{
		...
		mitigate_data.fm_cfg.side = inj_side;
		...
		u16 pn_thr)
{
	...
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Channel search failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmrx_hw_channel_search_exit;
	}

	{
		...
		err = fmrx_hw_get_fw_channel_info(fmrx_plat, &data);
		if (0 != err) {
			fmtrx_sys_log(CRIT,
		...
		fmtrx_sys_log
			(INFO, "FMR_HW: Frequency %d\n",
			data.frequency);
		fmtrx_sys_log
			(INFO, "FMR_HW: LO frequency %d\n",
			data.lo_freq);
		fmtrx_sys_log
			(INFO, "FMR_HW: VCO frequency %d\n",
			data.vco_freq);
		fmtrx_sys_log
			(INFO, "FMR_HW: LSI RSSI %d\n",
			RSSI_TO_DBUV(data.lsi_rssi));
		fmtrx_sys_log
			(INFO, "FMR_HW: LSI IMG RSSI %d\n",
			RSSI_TO_DBUV(data.lsi_img_rssi));
		fmtrx_sys_log
			(INFO, "FMR_HW: HSI RSSI %d\n",
			RSSI_TO_DBUV(data.hsi_rssi));
		fmtrx_sys_log
			(INFO, "FMR_HW: HSI IMG RSSI %d\n",
		...
		fmtrx_sys_log
			(INFO, "FMR_HW: Injection side : %s\n",
			(side == INJECTION_SIDE_AUTO)?"AUTO" :
			(side == INJECTION_SIDE_LSI)?"Low":"High");
	}
		...
		struct channel_info *data)
{
	...
	err = fmtrx_sys_reg_read32(fmrx_plat,
				FMR_RXMAIN_FREQTRACK_FOFFS_HZ_ADDR,
				(u32 *)&data->foffs);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Read 32-bit failed! %d\n",
	...
	err = fmrx_hw_get_fw_state(fmrx_plat, &data->state);
	if (0 != err) {
		fmtrx_sys_log
		...
		u32 *interrupt_status)
{
	...
	err = fmtrx_read32_masked(fmrx_plat, INTSTATUS_ADDR,
						interrupt_status,
	...
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Read 32-bit masked failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
		...
		u16 *groups_read)
{
	...
	fmtrx_sys_log
	(INFO, "%s: %s %d,Requested RDS groups %d!\n",
	FILE, __func__,
	...
	/* Get total RDS buffer size, start/end/host_read/write
			address from FW */
	err = fmrx_hw_get_rds_fw_buffer_info(fmrx_plat, &buf_info);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Get RDS buffer info failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmrx_hw_get_rds_groups_exit;
	}
	...
	fmtrx_sys_log
	(INFO, "%s: %s %d,RDS Buffer info,Host - 0x%x,Write - 0x%x!\n",
	FILE, __func__,
	__LINE__, buf_info.host_read_ptr,
	buf_info.fw_write_ptr);

	/* Get available number of RDS groups */
	err = fmrx_hw_get_rds_count_status(fmrx_plat,
			&buf_info, &avail_group_count,
				0);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Get RDS group count failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		err = -EINVAL;
		goto fmrx_hw_get_rds_groups_exit;
	...
	fmtrx_sys_log
	(INFO, "%s: %s %d,Available RDS groups - %d!\n",
	FILE, __func__,
	...
	fmtrx_sys_log
	(INFO, "%s: %s %d,No.of RDS groups to be read - %d!\n",
	FILE, __func__,
	__LINE__, groups_to_read);
	...
	err = fmtrx_sys_mem_read(fmrx_plat,
			buf_info.host_read_ptr,
			(u8 *)rds_groups, groups_part1 *
			FW_RDS_GROUP_SIZE);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,FMR IP Memory read failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmrx_hw_get_rds_groups_exit;
	}
	...
	fmtrx_sys_log
	(INFO, "%s: %s %d,New host offset - 0x%x!\n",
	FILE, __func__,
	__LINE__, buf_info.host_read_ptr);
		...
		s16 *data)
{
	...
	if (rds_sync) {
...
#ifdef FMR_INTR_MODE
		/* Enable RDS event */
		err = fmtrx_hw_set_interrupts(fmrx_plat, IR_SWINT1, true);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Set interrupt failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmrx_hw_get_rds_pi_exit;
		}
		...
		err = fmtrx_sys_wait_for_event(fmrx_plat,
				FMTRX_RDS_FAST_PI_TIMEOUTS);
		if (0 != err) {
			fmtrx_sys_log
			(CRIT, "%s: %s %d,Wait for Fast PI event failed! %d\n",
				FILE, __func__,
				__LINE__, err);
...
fmrx_hw_get_rds_pi_exit1:
#ifdef FMR_INTR_MODE
	{
		int err1 = 0;
		/* Disable RDS event */
		err1 = fmtrx_hw_set_interrupts(fmrx_plat, IR_SWINT1, false);
		if (0 != err1) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Set interrupt failed! %d\n",
				FILE, __func__,
	...
	struct debug_info *dbg_info)
{
	...
	err = fmrx_hw_get_fw_channel_info(fmrx_plat, &dbg_info->ch_info);
	if (0 != err) {
			fmtrx_sys_log(CRIT,
			"%s: %s %d,Get FW channel information failed! %d\n",
			FILE, __func__,
			__LINE__, err);
			goto fmrx_hw_get_debug_info_exit;
	}

	/* Fetch RDS buffer configuration details */
	err = fmrx_hw_get_rds_fw_buffer_info(fmrx_plat, &dbg_info->rds_info);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Get RDS buffer info failed! %d\n",
			FILE, __func__,
	...
	/* Fetch Channel tune/search/scan status information */
	err = fmrx_hw_get_channel_status_info(fmrx_plat,
			&dbg_info->status_info);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
			"%s: %s %d,Get channel status info failed! %d\n",
			FILE, __func__,
			 __LINE__, err);
		goto fmrx_hw_get_debug_info_exit;
	}

	/* Fetch Channel Scan buffer details */
	err = fmrx_hw_get_scan_buf_info(fmrx_plat, &dbg_info->scan_info);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
			"%s: %s %d,Get scan buffer info failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmrx_hw_get_debug_info_exit;
	}

fmrx_hw_get_debug_info_exit:
	return err;
}
		...
		struct channel_dbg_info *data)
{
	...
	err = fmtrx_sys_mem_read(fmrx_plat,
			FMR_RXMAIN_CH_INFO_CH_FREQ_ADDR,
			(u8 *)data, sizeof(struct channel_dbg_info));
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,FMR IP Memory read failed! %d\n",
			FILE, __func__,
	...
	struct ch_status_info *data)
{
	...
	if (NULL == data) {
		...
		err = -EINVAL;
		goto fmrx_hw_get_channel_status_info_exit;
	}
	...
	if (0 != err) {
		fmtrx_sys_log
		(CRIT, "%s: %s %d, Register read 16-bit failed! %d\n",
		FILE, __func__,
		 __LINE__, err);
		goto fmrx_hw_get_channel_status_info_exit;
	}
	...
	err = fmtrx_sys_reg_read16(fmrx_plat,
				FMR_RXMAIN_CH_SCAN_STATUS_ADDR,
				&data->scan_status);
	if (0 != err) {
		fmtrx_sys_log
		(CRIT, "%s: %s %d, Register read 16-bit failed! %d\n",
...
fmrx_hw_get_channel_status_info_exit:
	return err;
}

static int fmrx_hw_get_scan_buf_info(
		struct fmrx_platform *fmrx_plat,
		struct scan_buf_info *data)
{
	int err = 0;
	if (NULL == data) {
		fmtrx_sys_log(CRIT,
			"%s: %s, Invalid arguments!\n", FILE, __func__);
		err = -EINVAL;
		goto fmrx_hw_get_scan_buf_info_exit;
	}

	err = fmtrx_sys_reg_read16(fmrx_plat,
			FMR_RXMAIN_CH_SCAN_BUF_START_ADDR,
			&data->buf_start);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
			"%s: %s %d, Register read 16-bit failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmrx_hw_get_scan_buf_info_exit;
	}
	...
	err = fmtrx_sys_reg_read16(fmrx_plat,
			FMR_RXMAIN_CH_SCAN_BUF_END_ADDR,
			&data->buf_end);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
			"%s: %s %d, Register read 16-bit failed! %d\n",
	...
	err = fmtrx_sys_reg_read16(fmrx_plat,
			FMR_RXMAIN_CH_SCAN_WRITE_PTR_ADDR,
			&data->write_ptr);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
			"%s: %s %d, Register read 16-bit failed! %d\n",
	...
	err = fmtrx_sys_reg_read16(fmrx_plat,
			FMR_RXMAIN_CH_SCAN_HOST_READ_PTR_ADDR,
			&data->host_read_ptr);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
			"%s: %s %d, Register read 16-bit failed! %d\n",
	...
	err = fmtrx_sys_reg_read16(fmrx_plat,
			FMR_RXMAIN_CH_SCAN_HOST_IRQ_PTR_ADDR,
			&data->host_irq_ptr);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
			"%s: %s %d, Register read 16-bit failed! %d\n",
...
fmrx_hw_get_scan_buf_info_exit:
	return err;
}
		...
		u16 cmd_params_size)
{
	...
	err = fmtrx_hw_set_interrupts(fmrx_plat, ir_type, true);
	if (0 != err) {
		fmtrx_sys_log
		(CRIT, "%s: %s %d,Set Command complete interrupt failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmtrx_send_cmd_exit;
	}
	...
	/* Notify FW about new command in the command
			buffer */
	err = fmtrx_write_bit(fmrx_plat, TRIGSETCTL_ADDR,
			TRIGSETCTL_SWTRIGSET_POS32, 1);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Write bit failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmtrx_send_cmd_err1;
	}

	err = fmtrx_sys_wait_for_event(fmrx_plat, FMTRX_TIMEOUTS);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
...
#ifdef FMR_INTR_MODE
	{
		int err1 = fmtrx_hw_set_interrupts(fmrx_plat, ir_type,
					false);
		if (0 != err1) {
			fmtrx_sys_log(CRIT,
		...
		bool interrupt_enable)
{
	...
	if (interrupt_enable) {
		...
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Enable IRQ failed! %d\n",
				FILE, __func__,
				__LINE__, err);
	...
	err = fmtrx_sys_reg_write32(fmrx_plat, INTMASK_ADDR,
...
fmtrx_hw_set_interrupts_err1:
	...
	{
		int err1 = 0;
		err1 = fmtrx_sys_irq_disable(fmrx_plat);
		if (0 != err1) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Disable IRQ failed! %d\n",
				FILE, __func__,
				__LINE__, err1);
		}
	}
		...
		u32 value)
{
	...
	err = fmtrx_write_field(fmrx_plat, addroffs,
				pos, 1, value);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Write bit field failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
		...
		u32 value)
{
	...
	err = fmtrx_sys_reg_read32(fmrx_plat, addroffs, &val_tmp);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Read 32-bit failed! %d\n",
			FILE, __func__,
	...
	err = fmtrx_sys_reg_write32(fmrx_plat, addroffs, val_tmp);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Write 32-bit failed! %d\n",
			FILE, __func__,
		...
		u32 mask)
{
	...
	err = fmtrx_sys_reg_read32(fmrx_plat, addroffs, &tmp_mask);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Read 32-bit failed! %d\n",
			FILE, __func__,
