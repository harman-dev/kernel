...
retry:
	...
	if ((data->info_crc == 0) || (data->info_crc != calculated_crc)) {
		...
		if (count < MAX_RETRY) { /* Retry to read CRC if mismatch by data corruption */
			count++;
			goto retry;
		}
		dev_err(&client->dev, "CRC read retry failed %d times\n", count);
		error = -EIO;
		goto err_free_mem;
	}
...
static ssize_t mxt_touch_disable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	char c;

	c = data->touch_disabled ? '1' : '0';
	return scnprintf(buf, PAGE_SIZE, "%c\n", c);
}

static ssize_t mxt_touch_disable_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtou8(buf, 0, &i) == 0 ) {
		i = !!i;
		if(data->touch_disabled != i) {
			data->touch_disabled = i;
			if (data->touch_disabled) {
				disable_irq(data->irq);
				dev_info(dev, "Disabled Touch Screen\n");
			} else {
				mxt_acquire_irq(data);
				dev_info(dev, "Enabled Touch Screen\n");
			}
		} else {
			dev_info(dev, "%s Already Nothing to be Done\n", i ? "Touch Disbled" : "Touch Enabled");
		}
	} else {
		dev_err(dev, "mxt_touch_disable_store: error while copy data from user\n");
		return -EINVAL;
	}
...
static DEVICE_ATTR(touch_disable, S_IWUSR | S_IRUGO, mxt_touch_disable_show, mxt_touch_disable_store);
		...
		const struct i2c_device_id *id)
{
	...
	if (!data->pdata) {
		...
		gpio = devm_gpiod_get_index(&client->dev, "atml_gpio_rst", 0,GPIOD_ASIS);
		if (!IS_ERR(gpio)) {
		...
		gpio = devm_gpiod_get_index(&client->dev, "atml_gpio_int", 2,GPIOD_ASIS);
		if (!IS_ERR(gpio)) {
	...
	client->irq =
		gpiod_to_irq(gpio_to_desc(data->pdata->gpio_int));
...
static int __init at_mxt_modinit(void)
{
        int ret = 0;
     	...
     	printk("\t[ATMEL MXT] %s(%d)\n", __FUNCTION__,__LINE__);

        ret = i2c_add_driver(&mxt_driver);
        if( ret != 0 ){
                printk(KERN_ERR "Faild to register ATMEL MXT I2C driver: %d\n", ret);
        }
        printk("\t[ATMEL MXT] %s(%d) return %d\n", __FUNCTION__,__LINE__, ret);
        return ret;
}
