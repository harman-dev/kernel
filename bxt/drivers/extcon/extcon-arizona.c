...
static irqreturn_t arizona_hpdet_irq(int irq, void *data)
{
	...
	/* If there was no mic previously and a headphone was detected,
	 * only report headphone
	 */
	if (!info->mic) {
		if (report == EXTCON_JACK_HEADPHONE) {
			input_report_switch(info->input, SW_HEADPHONE_INSERT, 1);
			input_sync(info->input);
			dev_dbg(arizona->dev, "Report HP :%s\n", __func__);
		}
	}
...
static void arizona_micd_detect(struct work_struct *work)
{
	...
	if (info->detecting && (val & ARIZONA_MICD_LVL_8)) {
		...
		input_sync(info->input);
		dev_dbg(arizona->dev, "Report HS: %s\n", __func__);
...
static irqreturn_t arizona_jackdet(int irq, void *data)
{
	...
	if (info->last_jackdet == present) {
		...
		/* If mic was detected previously send mic removal event */
		if (info->mic)
			input_report_switch(info->input, SW_MICROPHONE_INSERT, 0);

		dev_dbg(arizona->dev, "Report Jack Removal : %s\n", __func__);
