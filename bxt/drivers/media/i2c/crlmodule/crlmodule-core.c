...
#if (DEBUG_954)
        if(sensor->sensor_ds->sensor_status_read)
		sensor->sensor_ds->sensor_status_read(client);
#endif
	return 0;
}
...
static int crlmodule_registered(struct v4l2_subdev *subdev)
{
	...
	if (pm_runtime_get_sync(&client->dev) < 0) {
        ...
        /* custom sensor specific init */
        if (sensor->sensor_ds->custom_sensor_init) {
                rval = sensor->sensor_ds->custom_sensor_init(client);
                if (rval) {
                        dev_err(&client->dev, "%s failed to run sensor specific custom init\n",
                                      __func__);
                        return -ENODEV;
                }
        }
