...
static S32 linuxos_Map_Kernel_Modules (void)
{
    ...
    SEP_PRINT_DEBUG("     kernel module            address           size\n");
    SEP_PRINT_DEBUG("  -------------------    ------------------    -------\n");
    ...
    linuxos_Load_Image_Notify_Routine("vmlinux",
                                      ...
                                      MR_SEG_NUM,
                                      1);
    SEP_PRINT_DEBUG("kmodule: %20s    0x%llx    0x%llx\n", "vmlinux", addr, size);
...
)
{
    ...
    SEP_PRINT_DEBUG("enter: unmap: hook_state %d abnormal %d\n", atomic_read(&hook_state), abnormal_terminate);
    ...
    if (!atomic_add_negative(1, &hook_state)) {
        SEP_PRINT_DEBUG("unmap: hook_state %d abnormal %d\n", atomic_read(&hook_state), abnormal_terminate);
        mm = current->mm;
    ...
    SEP_PRINT_DEBUG("exit: unmap done: hook_state %d\n", atomic_read(&hook_state));
    return 0;
}
...
)
{
    ...
    SEP_PRINT_DEBUG("exit_task done: hook_state %d\n", atomic_read(&hook_state));
...
capture_sched_switch(void * p)
{
    ...
    sideband_info = (SIDEBAND_INFO)OUTPUT_Reserve_Buffer_Space(bd, sizeof(SIDEBAND_INFO_NODE), FALSE);
...
#endif
{
    ...
    SEP_PRINT_DEBUG("[CPU %d, 0x%llx: [ OUT<%d:%d:%s> - IN<%d:%d:%s> ]]\n",
                    this_cpu, tsc, from->tgid, from->pid, from->comm, to->tgid, to->pid, to->comm);
...
)
{
    ...
    if (tp && ptp) {
        SEP_PRINT_DEBUG("trace point name: %s\n", tp->name);
        if (!strcmp(tp->name, "sched_switch")) {
            SEP_PRINT_DEBUG("Find trace point for sched_switch\n");
            *ptp = tp;
        }
    }
}
...
{
    int err = 0;
    SEP_PRINT_DEBUG("Installing PEBS linux OS Hooks\n");
    ...
    if (!tp_sched_switch) {
        err = -EIO;
        SEP_PRINT_DEBUG("Please make sure Linux kernel image is built with CONFIG_CONTEXT_SWITCH_TRACER\n");
    }
    else {
...
#else
    SEP_PRINT_DEBUG("Please use Linux kernel version >= 2.6.28 to use multiple pebs\n");
    err = -1;
#endif
...
)
{
    ...
    if (multi_pebs_enabled) {
        ...
        if (err) {
            SEP_PRINT_WARNING("Fail to install sched_switch callback for multiple pebs\n");
        }
    }
...
{
    int err = 0;
    SEP_PRINT_DEBUG("Uninstalling PEBS linux OS Hooks\n");
...
)
{
    ...
    if (multi_pebs_enabled) {
        ...
        if (err) {
            SEP_PRINT_WARNING("Fail to uninstall sched_switch callback for multiple pebs\n");
        }
    }
    ...
    if ((value == HOOK_FREE) || (value == HOOK_UNINSTALL)) {  // already in free or uninstall state
        SEP_PRINT_DEBUG("uninstall hook done -- already in state %d \n", value);
        return;
    }
    ...
    SEP_PRINT_DEBUG("uninstall hook done -- state %d, tries %d\n", value, tries);
    return;
}
...
)
{
    ...
    FOR_EACH_TASK(p) {
        ...
        if (!strncmp(p->comm, "qemu-kvm", 8)) {
            SEP_PRINT_DEBUG("Found qemu-kvm!\n");
            return TRUE;
        }
    }
