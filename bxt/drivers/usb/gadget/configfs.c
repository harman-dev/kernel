...
static int android_device_create(struct gadget_info *gi)
{
	...
	/*Check if the Android device has been created already*/
	if (android_device)
		return 0;
...
static void android_device_destroy(struct config_item *item)
{
	...
	/*Attempt to destroy the adnroid0 device any after checking
	the gadget info to which the device is assiciated*/
	if (!(android_device && (gi == dev_get_drvdata(android_device))))
		return;

	if (gi != to_gadget_info(item))
		return;
