...
static int dwc3_core_init(struct dwc3 *dwc)
{
	...
	case DWC3_GHWPARAMS1_EN_PWROPT_HIB:
		/*
		 * On bxtp platform we find sometimes PMU might fail to control
		 * the PHY when runtime pm schedule the DWC3 resume if the
		 * hibernation feature is enabled. Disable the hibernation
		 * can ensure DWC3 resume switch from D3 to D0 successfully.
		 */
		if (!platform_is_bxtp()) {
		...
		unsigned long event, void *data)
{
	...
	case USB_EVENT_NONE:
		...
		break;
	default:
		dev_dbg(dwc->dev, "DWC3 OTG Notify unknown notify message\n");
...
static int dwc3_probe(struct platform_device *pdev)
{
	...
	/*
	 * xxx: this should be set in pdata, but for BXT-P we don't use it
	 * fix this line once we handle pdata properly
	 */
	dwc->runtime_suspend = true;
	...
	if (dwc->runtime_suspend) {
		/*
		 * Workaround for permanent suspend in recovery mode
		 * restore this 500 ms delay using init script, after
		 * device enumerates successfully
...
static int dwc3_suspend_common(struct device *dev)
{
	...
	if (platform_is_bxtp()) {
		reg = dwc3_readl(dwc->regs, OTG_APBFC_U3PMU_CFG4);
		reg |= BIT(0) | BIT(1);
		dwc3_writel(dwc->regs, OTG_APBFC_U3PMU_CFG4, reg);
		while (--retries) {
			reg = dwc3_readl(dwc->regs, OTG_APBFC_U3PMU_CFG2);
			if ((reg & OTG_POWER_STATE_D3) == OTG_POWER_STATE_D3)
				break;
			else
				udelay(5);
		}
