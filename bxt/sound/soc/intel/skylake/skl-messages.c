...
int skl_resume_dsp(struct skl *skl)
{
	...
	skl_dsp_set_dma_clk_controls(skl->skl_sst, &clk_cnt);
				...
				u32 caps_size, u32 node_id)
{
	 ...
	 * present so return
	 */
	if (caps_size == sizeof(config_blob))
		return -EINVAL;
...
int skl_dsp_set_dma_clk_controls(struct skl_sst *ctx, int *clk_cnt)
{
	struct skl_dfw_dma_ctrl_hdr *hdr;
	struct nhlt_specific_cfg *cfg = NULL;
	struct skl_dfw_manifest *minfo = &ctx->manifest;
	...
	while (sz_remaining != 0) {
		...
		/* get nhlt specifi config info */
		cfg = skl_get_nhlt_specific_cfg(skl, hdr->vbus_id,
					NHLT_LINK_SSP, hdr->fmt.bit_depth,
					hdr->fmt.channels, hdr->fmt.freq,
					hdr->direction);
		...
		if (cfg && hdr->data_size) {
			...
			print_hex_dump(KERN_INFO, "Blob + DMA Control Info:",
			...
			dev_info(ctx->dev, "Failed to load NHLT specific Configuration\n");
			return -EIO;
		}
