...
static void dpm_psy_worker(struct work_struct *work)
{
	...
	if (!psy)
		return;
	ret = psy->desc->get_property(psy, POWER_SUPPLY_PROP_CAPACITY, &val);
	if (ret) {
					...
					enum charger_mode mode)
{
	int ret = 0;
	struct power_supply *psy;
	static union power_supply_propval val;
	...
	case CHARGER_MODE_DISABLE:
		...
		ret = psy->desc->set_property(psy,
				POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &tmp);
		if (ret < 0)
