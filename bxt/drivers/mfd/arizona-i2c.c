...
static int imp_offset = 70;
module_param(imp_offset, int, 0644);
MODULE_PARM_DESC(imp_offset, "Extra impedance in the MIC detect path");
...
static struct arizona_micd_range micd_ctp_ranges[] = {
	{ .max =  88, .key = KEY_MEDIA },
	{ .max = 196, .key = KEY_VOICECOMMAND },
	{ .max = 321, .key = KEY_VOLUMEUP },
	{ .max = 903, .key = KEY_VOLUMEDOWN },
};
			     ...
			     const struct i2c_device_id *id)
{
	...
	case WM8280:
		...
		if (i2c->irq < 0) {
			...
			irq_desc = devm_gpiod_get(&i2c->dev, NULL,
						  GPIOD_ASIS);
			if (IS_ERR(irq_desc))
				goto gpio_error;
			else
	...
	case WM1814:
		...
		}
		{
			...
			for (i = 0; i < ARRAY_SIZE(micd_ctp_ranges); i++)
				micd_ctp_ranges[i].max += imp_offset;
		}
...
gpio_error:
	dev_err(&i2c->dev, "Unable to get gpio pin\n");
	return -EINVAL;
}
