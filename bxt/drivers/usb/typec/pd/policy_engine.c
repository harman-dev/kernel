...
{
	log_dbg("GCRC in State-%d", pe->cur_state);
	switch (pe->cur_state) {
...
pe_process_state_pe_snk_ready(struct policy_engine *pe)
{
	...
	if (pe->prev_state == PE_SNK_SELECT_CAPABILITY
		...
		/* Set new inlimit */
		devpolicy_update_charger(pe->p.dpm, rcap->op_ma * 1000, 0);
		pe_store_port_partner_caps(pe, &pe->pp_src_pdos);
		schedule_delayed_work(&pe->post_ready_work,
				msecs_to_jiffies(PE_SNK_AUTO_TRIGGERING_DELAY));
	} else
		schedule_delayed_work(&pe->post_ready_work,
			pe->last_vdm_error ? PE_SNK_AUTO_TRIGGERING_DELAY : 0);
}
...
static void pe_process_state_pe_src_hard_reset(struct policy_engine *pe)
{
	...
	pe_start_timer(pe, PS_HARD_RESET_TIMER, PE_TIME_PS_HARD_RESET);
	pe_do_pe_reset(pe);
}
