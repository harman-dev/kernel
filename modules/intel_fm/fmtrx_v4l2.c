		...
		enum fmtrx_type type)
{
	...
	if (0 == v4l2_dev) {
		err = -ENOMEM;
		fmtrx_sys_log
			(CRIT, "%s: %s %d,V4L2 device allocn failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmtrx_v4l2_init_exit;
	}
	...
	/* Data structures */
	mutex_init(&radio->interface_lock);

	/* Register with FMR_V4L2 subsystem */
	err = v4l2_device_register(radio->dev.parent, v4l2_dev);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,V4L2 device reg failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmtrx_v4l2_init_exit1;
	}

	radio->video_dev.v4l2_dev = v4l2_dev;
	radio->video_dev.fops = &fmtrx_v4l2_fops;
	radio->video_dev.ioctl_ops = &fmtrx_v4l2_iops;
	radio->video_dev.release = video_device_release_empty;

	/* Register video device */
	err = video_register_device(&radio->video_dev,
			VFL_TYPE_RADIO, -1);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,V4L2 video device reg failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		...
		enum fmtrx_type type)
{
	...
	if (0 != radio->video_dev.v4l2_dev) {
		/* Unregister video device */
		video_unregister_device(&radio->video_dev);

		/* Unregister v4l2 device */
		v4l2_device_unregister(radio->video_dev.v4l2_dev);

		kfree(radio->video_dev.v4l2_dev);
	}
		...
		struct file *fp)
{
	...
	struct fmtrx_radio_device *radio = video_get_drvdata(video_devdata(fp));

	mutex_lock(&radio->interface_lock);

	err = v4l2_fh_open(fp);
	if (err < 0) {
		fmtrx_sys_log
			(INFO, "%s: %s %d, V4L2 FH open failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	...
	/* Initialize FM core module only once
			for a open call */
	if (v4l2_fh_is_singular_file(fp)) {
		/* Initialize the FM core module */
		err = fmtrx_init(&radio->data, FMTRX_INIT_MODE_ON, FMTRX_RX);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,FMTRX core init failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmtrx_v4l2_fops_open_exit;
		}

		radio->data->fmrx_plat->dev.parent = radio->dev.parent;

		err = fmtrx_sys_get_rx_default_config(radio->data->fmrx_plat,
					&radio->data->fmrx_cfg);
		if (0 != err) {
			fmtrx_sys_log(CRIT,
		...
		err = fmrx_power_on(radio->data);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,FM RX power on failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmtrx_v4l2_fops_open_exit1;
		}
		...
		fmtrx_sys_log
			(INFO, "%s: %s %d, Device is already opened! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
	goto fmtrx_v4l2_fops_open_exit;
...
fmtrx_v4l2_fops_open_exit1: {
		err1 = fmtrx_init(
				&radio->data, FMTRX_INIT_MODE_OFF, FMTRX_RX);
		if (0 != err1) {
			fmtrx_sys_log(CRIT,
...
fmtrx_v4l2_fops_open_exit:
	if (err != 0)
		v4l2_fh_release(fp);
	mutex_unlock(&radio->interface_lock);
	return err;
}
...
{
	int err = 0;
	u16 requested_groups = 0, tries = 1,
			total_read_bytes = 0;
	...
	err = fmrx_get_hw_state(radio->data, &state);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Get hw state failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmtrx_v4l2_fops_read_exit;
	}
	...
	if (FMRX_HW_STATE_RX_ACTIVE != state) {
		err = -EBADFD;
		fmtrx_sys_log
		(CRIT, "%s: %s %d,FMR in wrong state! Current state: %d, %d\n",
			FILE, __func__,
			__LINE__, state, err);
		goto fmtrx_v4l2_fops_read_exit;
	}

	if (RDS_ONMODE_OFF == radio->data->fmrx_cfg->rds_cfg.mode) {
		struct rds rds_cfg = { RDS_ONMODE_ON };

		err = fmrx_set_rds(radio->data, &rds_cfg);
		if (0 != err) {
			fmtrx_sys_log
	...
	do {
		err = fmrx_get_rds_groups(radio->data, &rds_pkt);
		if (0 != err) {
			fmtrx_sys_log
	...
	if (0 != rds_pkt.received_groups) {
		...
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Copy to user failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			err = -EAGAIN;
		}
		...
		struct file *fp)
{
	...
	/* Cleanup only when there is one Application
		holding the driver */
	if (v4l2_fh_is_singular_file(fp)) {
		err = fmrx_power_off(radio->data);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,FM RX power off failed! %d\n",
				FILE, __func__,
				__LINE__, err);
		}
		...
		unsigned long arg)
{
	...
	fmtrx_sys_log
		(INFO, "%s: %s %d,32-bit compatibility layer invoked!\n",
		FILE, __func__,
		__LINE__);
	...
	case V4L2_CID_PRIV_INTEL_FMRX_BAND: {
		struct band_32 __user *_data32 = (struct band_32 *)user;
		struct band _data64;

		/* Copy from user space structure */
		err = get_user(_data64.max, &_data32->max);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Get user data failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmtrx_v4l2_fops_compat_ioctl32_exit;
		}

		err = get_user(_data64.min, &_data32->min);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Get user data failed! %d\n",
				FILE, __func__,
		...
		err = get_user(_data64.step, &_data32->step);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Get user data failed! %d\n",
				FILE, __func__,
		...
		err = get_user(_data64.deemp, &_data32->deemp);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Get user data failed! %d\n",
				FILE, __func__,
		...
		err = fmrx_set_band(radio->data, &_data64);
		if (0 != err) {
			fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_AF_SWITCH: {
		...
		err = fmrx_af_switch(radio->data, &_data64);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Switch to AF failed! %d\n",
				FILE, __func__,
				__LINE__, err);
		}
		break;
	...
	case V4L2_CID_PRIV_INTEL_FMTRX_WRITE: {
		...
		if (V4L2_CID_PRIV_INTEL_FMTRX_READ == cmd) {
			err = fmtrx_sys_reg_read32(radio->data->fmrx_plat,
					_data64.addr_offs, &value);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
			...
			if (0 != err) {
				fmtrx_sys_log
				(CRIT, "%s: %s %d,Copy from user failed! %d\n",
					FILE, __func__,
					__LINE__, err);
				goto fmtrx_v4l2_fops_compat_ioctl32_exit;
			}

			err = fmtrx_sys_reg_write32(radio->data->fmrx_plat,
					_data64.addr_offs, value);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
					"%s: %s %d,Read 32bit failed! %d\n",
	...
	case V4L2_CID_PRIV_INTEL_FMRX_RDS_DATA: {
		...
		err = fmrx_get_rds_groups(radio->data, &_data64);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Get RDS groups failed! %d\n",
				FILE, __func__,
	...
	default: {
		...
		if (0 != err) {
			fmtrx_sys_log
			(CRIT, "%s: %s %d,Private IOCTL execution failed! %d\n",
				FILE, __func__,
				__LINE__, err);
		}
		break;
		...
		struct v4l2_tuner *data)
{
	...
	err = fmrx_get_band(radio->data, &band_cfg);
	if (0 != err) {
		err = -EIO;
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Fetch band information failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmtrx_v4l2_iops_gtuner_exit;
	}
	...
	err = fmrx_get_channel_info(radio->data, &ch_info);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,FM get channel info failed! %d\n",
			FILE, __func__,
			__LINE__, err);
		goto fmtrx_v4l2_iops_gtuner_exit;
	}
	...
	data->audmode = (radio->data->fmrx_cfg->force_mono) ?
		...
		const struct v4l2_frequency *data)
{
	...
	mutex_lock(&radio->interface_lock);
	err = fmrx_station_tuning(radio->data, frequency);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,Set station tuning failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
	mutex_unlock(&radio->interface_lock);
		...
		struct v4l2_frequency *data)
{
	...
	err = fmrx_get_channel_freq(radio->data, &tuned_frequency);
	if (0 != err) {
		fmtrx_sys_log(CRIT,
		...
		struct v4l2_control *data)
{
	...
	switch (data->id) {
	case V4L2_CID_AUDIO_MUTE:
		data->value = radio->data->fmrx_cfg->mute;
		break;
	case V4L2_CID_AUDIO_VOLUME:
		data->value = radio->data->fmrx_cfg->vol_cfg.left;
		break;
	case V4L2_CID_TUNE_DEEMPHASIS:
		data->value = radio->data->fmrx_cfg->band_cfg.deemp;
		break;
	case V4L2_CID_PRIV_INTEL_FMRX_OUTPUT_MODE:
		data->value = radio->data->fmrx_cfg->routing;
		break;
	case V4L2_CID_PRIV_INTEL_FMRX_ANTENNA:
		data->value = radio->data->fmrx_cfg->antenna;
		break;
	case V4L2_CID_PRIV_INTEL_FMRX_SB:
		data->value = radio->data->fmrx_cfg->side;
		break;
	default:
		err = -EINVAL;
		break;
		...
		struct v4l2_control *data)
{
	...
	case V4L2_CID_AUDIO_VOLUME:
		...
		err = fmrx_set_volume_level(radio->data,
						FMRX_AUD_CHN_ALL, data->value);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,FM Set volume failed! %d\n",
	...
	case V4L2_CID_AUDIO_MUTE:
		...
		err = fmrx_set_mute(radio->data, FMRX_AUD_CHN_ALL,
					data->value);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,FM Set mute failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmtrx_v4l2_iops_sctrl_exit;
		}
	...
	case V4L2_CID_TUNE_DEEMPHASIS: {
		...
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,Get band config failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmtrx_v4l2_iops_sctrl_exit;
		}
		...
		err = fmrx_set_band(radio->data, &band_cfg);
		if (0 != err) {
			fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_OUTPUT_MODE:
		err = fmrx_set_route(radio->data, data->value);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,FM Set route failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmtrx_v4l2_iops_sctrl_exit;
		}
		break;
	case V4L2_CID_PRIV_INTEL_FMRX_ANTENNA:
		err = fmrx_set_antenna(radio->data, data->value);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,FM Set antenna failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmtrx_v4l2_iops_sctrl_exit;
		}
		break;
	case V4L2_CID_PRIV_INTEL_FMRX_SB:
		err = fmrx_set_sideband(radio->data,
				data->value, true);
		if (0 != err) {
			fmtrx_sys_log
				(CRIT, "%s: %s %d,FM Set sideband failed! %d\n",
				FILE, __func__,
				__LINE__, err);
			goto fmtrx_v4l2_iops_sctrl_exit;
		}
		...
		const struct v4l2_hw_freq_seek *data)
{
	...
	/* reserved[1] will have the signal level
		to detect */
	err = fmrx_station_seeking(radio->data, data);
	if (0 != err) {
		fmtrx_sys_log
			(CRIT, "%s: %s %d,FM station seeking failed! %d\n",
			FILE, __func__,
			__LINE__, err);
	}
	mutex_unlock(&radio->interface_lock);
		...
		void *data)
{
	...
	case V4L2_CID_PRIV_INTEL_FMRX_SNC: {
		...
		if (likely(access_ok(VERIFY_READ, snc_cfg,
					(sizeof(struct snc))))) {
			err = fmrx_set_snc(radio->data, snc_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_SM: {
		...
		if (likely(access_ok(VERIFY_READ, sm_cfg,
					(sizeof(struct sm))))) {
			err = fmrx_set_sm(radio->data, sm_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_AGC: {
		...
		if (likely(access_ok(VERIFY_READ, agc_cfg,
				(sizeof(struct agc))))) {
			err = fmrx_set_agc(radio->data, agc_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_RSSI: {
		...
		if (likely(access_ok(VERIFY_READ, rssi_cfg,
			...
			err = fmrx_set_rssi_notification(radio->data,
							rssi_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_SET_HWPARAMS: {
		...
		if (likely(access_ok(VERIFY_READ, other_cfg,
			...
			err = fmrx_set_other_params(radio->data,
							other_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_GET_HWPARAMS: {
		...
		if (likely(access_ok(VERIFY_READ, other_cfg,
					(sizeof(struct other_params))))) {
			memcpy((u8 *)other_cfg,
					(u8 *)&radio->data->fmrx_cfg->other_cfg,
					sizeof(struct other_params));
			err = 0;
		}
		break;
	...
	case V4L2_CID_PRIV_INTEL_FMRX_STATIC_CFG: {
		...
		if (likely(access_ok(VERIFY_READ, fmrx_cfg,
			...
			memcpy((u8 *)fmrx_cfg, (u8 *)radio->data->fmrx_cfg,
						sizeof(struct fmrx_config));
			err = 0;
		}
		break;
	...
	case V4L2_CID_PRIV_INTEL_FMTRX_WRITE: {
		...
		if (likely(access_ok(VERIFY_READ, reg_cfg,
				(sizeof(struct reg_info))))) {
			err = fmtrx_sys_reg_write32(radio->data->fmrx_plat,
					reg_cfg->addr_offs, *reg_cfg->data);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
					"%s: %s %d,Read 32bit failed! %d\n",
					FILE, __func__,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_SET_OFFSETS: {
		...
		if (likely(access_ok(VERIFY_READ, gain_offsets_pkt_cfg,
			...
			err = fmtrx_set_gain_offsets(radio->data,
							gain_offsets_pkt_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_GET_OFFSETS: {
		...
		if (likely(access_ok(VERIFY_READ, gain_offsets_pkt_cfg,
				(sizeof(struct gain_offsets_pkt))))) {
			err = fmtrx_get_gain_offsets(radio->data,
							gain_offsets_pkt_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_SET_RSSI_OFFSETS: {
		...
		if (likely(access_ok(VERIFY_READ, rssi_offsets_pkt_cfg,
			...
			err = fmtrx_set_gain_rssi_offsets(radio->data,
						rssi_offsets_pkt_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
					"%s: %s %d,Set RSSI Band offsets failed! %d\n",
					FILE, __func__,
					__LINE__, err);
				goto fmtrx_v4l2_iops_priv_exit;
			}
	...
	case V4L2_CID_PRIV_INTEL_FMRX_GET_RSSI_OFFSETS: {
		...
		if (likely(access_ok(VERIFY_READ, rssi_offsets_pkt_cfg,
				(sizeof(struct rssi_offsets_pkt))))) {
			err = fmtrx_get_gain_rssi_offsets(radio->data,
						rssi_offsets_pkt_cfg);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
					"%s: %s %d,Get RSSI Band offsets failed! %d\n",
					FILE, __func__,
					__LINE__, err);
				goto fmtrx_v4l2_iops_priv_exit;
			}
	...
	case V4L2_CID_PRIV_INTEL_FMRX_AF_SWITCH: {
		...
		if (likely(access_ok(VERIFY_READ, af,
			...
			err = fmrx_af_switch(radio->data, af);
			if (0 != err) {
				fmtrx_sys_log
				(CRIT, "%s: %s %d,Switch to AF failed! %d\n",
					FILE, __func__,
	...
	case V4L2_CID_PRIV_INTEL_FMTRX_GET_CORE_VERSION: {
		...
		if (likely(access_ok(VERIFY_READ, fmr_id_data,
			...
			err = fmtrx_get_id(radio->data, fmr_id_data);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
	...
	case V4L2_CID_PRIV_INTEL_FMRX_RDS_DATA: {
		...
		if (likely(access_ok(VERIFY_READ, rds_pkt,
			...
			err = fmrx_get_rds_groups(radio->data,
							rds_pkt);
			if (0 != err) {
				fmtrx_sys_log
				(CRIT, "%s: %s %d,Get RDS groups failed! %d\n",
	...
	case V4L2_CID_PRIV_INTEL_FMRX_GET_DEBUG_INFO: {
		struct debug_info *dbg_info  =
					(struct debug_info *) data;
		if (likely(access_ok(VERIFY_READ, dbg_info,
				(sizeof(struct debug_info))))) {
			err = fmrx_get_debug_info(radio->data, dbg_info);
			if (0 != err) {
				fmtrx_sys_log(CRIT,
				"%s: %s %d,FM Get debug info failed! %d\n",
					FILE, __func__,
					__LINE__, err);
				goto fmtrx_v4l2_iops_priv_exit;
			}
