...
static void pd_add_dummy_crc(struct pd_prot *pd)
{
	static struct pd_packet pkt;

	pkt.header.msg_type = PD_CTRL_MSG_GOODCRC;
	pd_prot_add_msg_rx_list(pd, &pkt, 2, PKT_TYPE_SOP);
}
...
static void pd_notify_protocol(struct typec_phy *phy, unsigned long event)
{
	...
	case PROT_PHY_EVENT_TX_SENT:
		dev_dbg(phy->dev, "%s: PROT_PHY_EVENT_TX_SENT\n",
				__func__);
		mutex_lock(&pd->tx_lock);
