...
enum {
	CPER_FWERR_TYPE_IPF_SAL,
	CPER_FWERR_TYPE_IFW_CRASHLOG,
};
...
struct cper_fw_err_rec_ref {
	__u8	error_type;
	__u8	reserved[7];
	__u64	identifier;
};

struct fw_err_type_crashlog {
	__u32	source;
	__u32	version;
};
...
const char *cper_mem_err_unpack(struct trace_seq *,
				struct cper_mem_err_compact *);
typedef void (cper_send_fw_data_func_t)(const struct cper_fw_err_rec_ref *,
					size_t);
