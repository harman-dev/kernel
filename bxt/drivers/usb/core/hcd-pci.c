...
#define PCI_USB_DEVICE_CONTROLLER_CLASS		0x0c03fe
		...
		companion_fn fn)
{
	...
	for_each_pci_dev(companion) {
		 ...
		 *
		 * On BXT Host Controller and Device Controller figure as
                 * same device with different device function. HCD should
                 * not pass data to Device Controller. Checking if
                 * companion device is Device Controller and omitting it.
		 */
		if ((companion->class != CL_UHCI &&
		     companion->class != CL_OHCI &&
		     companion->class != CL_EHCI) ||
		     companion->class == PCI_USB_DEVICE_CONTROLLER_CLASS)
			continue;
