...
static struct acpi_device_id intel_id_table[] = {
	...
	{ "INT33E3", (kernel_ulong_t)&acpi_default_bluetooth },
	{ },
};
...
enum bt_hci_nl_messages {
	...
	LD_DRIVER_REGISTER_FMR,
	/*BT PID Notification, From BT stack -> STD */
	LD_DRIVER_REGISTER_BT,
	...
	/* FORCE_INIT notification, From FMR stack -> STD*/
	BT_DEV_HARD_RESET_HW_ERR_RSP,
	BT_DEV_HARD_RESET_CMD_TO,
	/* FORCE_INIT notification, From FMR stack -> STD
				for FMR hci command timeout*/
	BT_BOOT_UP_BOOTLOADER_NOTIFY,
	/* HCI_Boot_up (bootloader mode) Notification, From STD -> BT stack */
	BT_BOOT_UP_OPERATIONAL_NOTIFY,
	/* HCI_boot_up (operational mode) Notification, From STD -> BT stack */
	BT_FLOW_SPEC_COMPL_SUCCESS,
	/* HCI Notification for batch compl success, From STD -> BT stack */
	BT_FLOW_SPEC_COMPL_FAILURE,
	/* HCI Notification for batch compl success, From STD -> BT stack */
};
...
static unsigned fmr_stack_pid;
static unsigned bt_stack_pid;
...
static long lbf_write(struct sk_buff *skb)
{
	...
	print_hex_dump_debug("<FMR out<", DUMP_PREFIX_NONE, 16, 1, skb->data,
			skb->len, 0);
	trace_lbf_bt_tx_buf(__func__, skb->data, skb->len);
...
static int lbf_send_netlink_message(
	struct tty_struct *tty, enum bt_hci_nl_messages code, unsigned int pid)
{
	...
	pr_info("%s: code %d for pid %d\n", __func__, code, pid);
	if ((!lbf_uart->nl_socket) || (pid == 0)) {
		err = INVALID;
		goto netlink_failed;
	}
	...
	err = nlmsg_unicast(lbf_uart->nl_socket, skb_out, pid);
...
static void lbf_netlink_callback(struct sk_buff *skb)
{
	...
	switch (data) {
	case LD_DRIVER_REGISTER_FMR:
		/* Process ID (PID) of sending process */
		trace_lbf_ldisc_info(__func__, "FMR registration");
		pr_debug("%s: FMR registration: pid %d",__func__, nlh->nlmsg_pid);
		fmr_stack_pid = nlh->nlmsg_pid;
		break;
	case LD_DRIVER_REGISTER_BT:
		/* Process ID (PID) of sending process */
		trace_lbf_ldisc_info(__func__, "BT registration");
		pr_debug("%s: BT registration: pid %d",__func__, nlh->nlmsg_pid);
		bt_stack_pid = nlh->nlmsg_pid;
		break;
					...
					struct lbf_uart *lbf_uart)
{
	...
	lbf_send_netlink_message(tty, BT_HCI_HW_ERR_REQUEST, fmr_stack_pid);
	ret = lbf_ldisc_send_hwerr_frame(tty, lbf_uart);
	return ret;
}

/* apply_hci_credit_workaround()
 * modify HCi credit value to 1 in operational mode
 * to prevent upper stack to send multiple HCI
 * commands in parallel (makes the bt firmware crash)
 * lbf_uart : Disc data for tty
 * Return Type: void
 */

static void apply_hci_credit_workaround(struct lbf_uart *lbf_uart)
{

#define HCI_CREDIT_INDEX_IN_HCI_COMMAND_COMPLETE_EVT 3
#define HCI_CREDIT_INDEX_IN_HCI_COMMAND_STATUS_EVT 4

      /*
         HCI credit hack:
         in operational mode, BT firmware does not support 2 HCI commands
         so force HCI credit to 1
      */

      int offset = 0;
      unsigned char * p_command_credit = NULL;
      unsigned char cmd = lbf_uart->rx_skb->data[1];

      if (HCI_COMMAND_COMPLETE_EVENT == cmd) {
            offset = HCI_CREDIT_INDEX_IN_HCI_COMMAND_COMPLETE_EVT;
      } else if (HCI_COMMAND_STATUS_EVT == cmd) {
            offset = HCI_CREDIT_INDEX_IN_HCI_COMMAND_STATUS_EVT;
      } else {
            /* do nothing */
            return;
      }

      p_command_credit = &lbf_uart->rx_skb->data[offset];

      if (*p_command_credit > 1) {
              pr_debug("lnp_ldisc: Forcing HCI credit from %d to 1 on command 0x%x",
                       *p_command_credit,
                       cmd);
              *p_command_credit = 1;
      }
...
static int st_send_frame(struct tty_struct *tty, struct lbf_uart *lbf_uart)
{
	...
	if (HCI_COMMAND_HARDWARE_ERROR == lbf_uart->rx_skb->data[1] &&
		!test_bit(HCI_CMD_TOUT, &lbf_uart->hci_cmd_st)) {
		pr_err("%s: Received H/W error!\n", __func__);
		/* inform both FMR and BT stack of H/W error event */
		lbf_send_netlink_message(tty, BT_HCI_HW_ERR_REQUEST, fmr_stack_pid);
		lbf_send_netlink_message(tty, BT_HCI_HW_ERR_REQUEST, bt_stack_pid);
		set_bit(HCI_HW_ERR, &lbf_uart->hci_cmd_st);
        ...
        if (fw_download_state == FW_SUCCESS) {
                apply_hci_credit_workaround(lbf_uart);
        }
	...
	if (HCI_COMMAND_COMPLETE_EVENT == lbf_uart->rx_skb->data[1]) {
		...
		case BT_HCI_RESET:
			...
			lbf_send_netlink_message(tty,
					BT_HCI_RESET_COMPLETE_NOTIFY, fmr_stack_pid);
			chnl_id = MODULE_BT;
	...
	if (HCI_VS_ASYNC_EVENT == lbf_uart->rx_skb->data[1]) {
		pr_debug("%s: Async event %d", __func__, lbf_uart->rx_skb->data[3]);
		switch (lbf_uart->rx_skb->data[3]) {
		case HCI_INTEL_SECURE_SEND_CMD_RESULT_ID:
			if (lbf_uart->rx_skb->data[4] == HCI_INTEL_SECURE_SEND_CMD_RES_SUCCESS) {
				lbf_send_netlink_message(tty,
					BT_FLOW_SPEC_COMPL_SUCCESS, bt_stack_pid);
				chnl_id = MODULE_BT;
				goto free_packet;
			}
			else {
				trace_lbf_ldisc_info(__func__,
						"Bootup error");
				lbf_send_netlink_message(tty,
					BT_FLOW_SPEC_COMPL_FAILURE, bt_stack_pid);
				pr_err("FW_TYPE: %x",
					lbf_uart->rx_skb->data[6]);
				pr_err("Bootup error: %x",
					lbf_uart->rx_skb->data[8]);
		...
		case HCI_INTEL_BOOTUP_EVENT_ID:
			...
			case OPERATIONAL:
				...
				case HCI_INTEL_BOOTUP_REASON_SYSTEM_EXCEP:
					...
					if (ret < 0)
						...
						goto free_packet;
				break;
				case HCI_INTEL_BOOTUP_REASON_UNKNW_RESET:
				case HCI_INTEL_BOOTUP_REASON_HCI_RESET:
				case HCI_INTEL_BOOTUP_REASON_HW_RESET:
				case HCI_INTEL_BOOTUP_REASON_INTEL_RESET:
					trace_lbf_ldisc_info(__func__,
						"Operational Bootup HCI event");
					pr_info("Operational Bootup HCI event");
					lbf_send_netlink_message(tty,
						BT_BOOT_UP_OPERATIONAL_NOTIFY, bt_stack_pid);
					chnl_id = MODULE_BT;
					goto free_packet;
				break;
				default:
			...
			case BOOTLOADER:
				...
				case HCI_INTEL_BOOTUP_REASON_UNKNW_RESET:
					trace_lbf_ldisc_info(__func__,
							"Bootloader Bootup HCI event");
					pr_info("Bootloader Bootup HCI event");
					lbf_send_netlink_message(tty,
						BT_BOOT_UP_BOOTLOADER_NOTIFY, bt_stack_pid);
					chnl_id = MODULE_BT;
					goto free_packet;
				break;
				case HCI_INTEL_BOOTUP_REASON_FATAL_EXCEP:
...
/* lbf_ldisc_hw_reset()
 * IOCTL to do a hard reset from independent module
 * Return Type: void
 */
static int lbf_ldisc_hw_reset(unsigned long arg)
{
	...
	if (fw_download_state == FW_SUCCESS) {
		...
		case MODULE_HW_ERR:
			...
			pr_err("H/W reset for Hardware error");
			break;
		case MODULE_CMD_TIMEOUT:
			if (!test_and_set_bit(HCI_CMD_TOUT,
						&lbf_uart->hci_cmd_st))
				pr_err("H/W reset for Bluetooth cmd timeout");
				lbf_send_netlink_message(lbf_tx->tty,
						BT_HCI_HW_ERR_REQUEST, fmr_stack_pid);
			trace_lbf_ldisc_info(__func__,
		...
		const unsigned char *data, size_t count)
{
	...
	int status = 0;
	unsigned short opcode = 0;
	const unsigned char *buff = data + 1;
	struct lbf_uart *lbf_uart = (struct lbf_uart *)tty->disc_data;

	trace_lbf_func_start(__func__);

	trace_lbf_bt_tx_buf(__func__, data, count);
	...
	STREAM_TO_UINT16(opcode, buff);
	pr_debug("%s: opcode: 0x%x", __func__, opcode);
	...
	if (ret == 0 && BT_HCI_RESET == opcode) {
		...
		status = lbf_send_netlink_message(tty, BT_HCI_RESET_REQUEST, fmr_stack_pid);
		if (status == INVALID)
...
#ifndef CONFIG_SERIAL_HSU
	intel_lbf_lpm.tty_dev = dev->parent;
	pr_info("%s: tty_dev= %d\n", __func__, (int) intel_lbf_lpm.tty_dev);
#endif
...
static int intel_bluetooth_pdata_probe(struct platform_device *pdev)
{
	...
	intel_lbf_lpm.gpiod_enable_bt = of_get_named_gpiod_flags(dn,
					PROP_LNP_BT_GPIO_ENB, 0, NULL);
	if (intel_lbf_lpm.gpiod_enable_bt <= 0) {
		pr_err("Can't find node %s\n", PROP_LNP_BT_GPIO_ENB);
		goto error;
	} else
		pr_debug("bt_fmr %d\n", intel_lbf_lpm.gpiod_enable_bt);
...
/* If Device tree Structure is supported*/
	...
	ret = gpio_request(desc_to_gpio(intel_lbf_lpm.gpiod_enable_bt), "bt-reset");
	if (ret < 0) {
	...
	pr_debug("%s: bt_gpio:%d/n", __func__, intel_lbf_lpm.gpiod_enable_bt);

	bt_rfkill_set_power(default_state);
	return ret;

err_gpio_enable_dir:
	gpio_free(desc_to_gpio(intel_lbf_lpm.gpiod_enable_bt));
err_gpio_enable_req:
#endif
