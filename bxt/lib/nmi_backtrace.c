				   ...
				   void (*raise)(cpumask_t *mask))
{
	...
	if (in_nmi() && include_self) {
		/*
		 * If already running in NMI context, the current CPU
		 * cannot respond to NMI as NMIs cannot be nested.
		 */
		...
		if (!cpumask_empty(to_cpumask(backtrace_mask)))
			...
			pr_info("Sending NMI to none CPUs\n");
	}
