...
wlan_pcie_send_data_complete(mlan_adapter *pmadapter)
{
	...
	while (((pmadapter->txbd_rdptr & TXRX_RW_PTR_MASK)
		!= (rdptr & TXRX_RW_PTR_MASK))
		   ...
		   != (rdptr & TXRX_RW_PTR_ROLLOVER_IND))) {
		wrdoneidx = pmadapter->txbd_rdptr & TXRX_RW_PTR_MASK;
		pmbuf = pmadapter->tx_buf_list[wrdoneidx];
		if (pmbuf) {
			PRINTM(MDAT_D,
			       "SEND DATA COMP: Detach pmbuf %p at txbd_rdidx=%d\n",
			       pmbuf, wrdoneidx);
		    ...
		    mlan_buffer *pmbuf, mlan_tx_param *tx_param)
{
	...
	if (PCIE_TXBD_NOT_FULL(pmadapter->txbd_wrptr, pmadapter->txbd_rdptr)) {
		...
		*tmp = wlan_cpu_to_le16(MLAN_TYPE_DATA);
		...
		wrindx = pmadapter->txbd_wrptr & TXRX_RW_PTR_MASK;
		PRINTM(MDAT_D, "SEND DATA: Attach pmbuf %p at txbd_wridx=%d\n",
		       pmbuf, wrindx);
...
wlan_pcie_process_recv_data(mlan_adapter *pmadapter)
{
	...
	pmadapter->rxbd_rdptr = rdptr;

	while (((rdptr & TXRX_RW_PTR_MASK) !=
		(pmadapter->rxbd_wrptr & TXRX_RW_PTR_MASK))
	       || ((rdptr & TXRX_RW_PTR_ROLLOVER_IND) ==
		   (pmadapter->rxbd_wrptr & TXRX_RW_PTR_ROLLOVER_IND))) {
		/* detach pmbuf (with data) from Rx Ring */
		rd_index = pmadapter->rxbd_wrptr & TXRX_RW_PTR_MASK;
		...
		PRINTM(MDAT_D, "RECV DATA: Detach pmbuf %p at rxbd_rdidx=%d\n",
		       pmbuf, rd_index);
		...
		PRINTM(MDAT_D,
		       "RECV DATA: Attach new pmbuf %p at rxbd_rdidx=%d\n",
		       pmbuf, rd_index);
...
wlan_pcie_process_event_ready(mlan_adapter *pmadapter)
{
	...
	if (wrptr >= MLAN_MAX_EVT_BD) {
		PRINTM(MINFO, "Invalid write pointer...\n");
		LEAVE();
		return MLAN_STATUS_FAILURE;
	}
	...
	if (((rdptr & EVT_RW_PTR_MASK) !=
		...
		pmbuf_evt = pmadapter->evt_buf_list[wrptr];

		/*unmap the pmbuf for CPU Access */
		pcb->moal_unmap_memory(pmadapter->pmoal_handle,
				       pmbuf_evt->pbuf + pmbuf_evt->data_offset,
				       pmadapter->evtbd_ring[wrptr]->paddr,
				       MAX_EVENT_SIZE, PCI_DMA_FROMDEVICE);

		/* Take the pointer and set it to event pointer in adapter
		   and will return back after event handling callback */
		pmadapter->evt_buf_list[wrptr] = MNULL;
		pmadapter->evtbd_ring[wrptr]->paddr = 0;
		pmadapter->evtbd_ring[wrptr]->len = 0;
...
wlan_interrupt(pmlan_adapter pmadapter)
{
	...
	if (pcb->moal_read_reg(pmoal_handle, PCIE_HOST_INT_STATUS, &pcie_ireg)) {
	...
	if ((pcie_ireg != 0xFFFFFFFF) && (pcie_ireg)) {
		...
		if (pcb->moal_write_reg(pmoal_handle, PCIE_HOST_INT_STATUS,
					~pcie_ireg)) {
