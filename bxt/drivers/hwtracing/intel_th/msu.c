...
static int msc_configure(struct msc *msc)
{
	...
	if (msc->enabled) {
		...
		pstore_register_extra_dumper(&msc->pstore_dumper);
	}
...
static void intel_th_msc_deactivate(struct intel_th_device *thdev)
{
	...
	msc_disable(msc);
	pstore_unregister_extra_dumper(&msc->pstore_dumper);
...
static int intel_th_msc_init(struct msc *msc)
{
	...
	if (msc->thdev->id == 0)
		msc->thdev->output.scratchpad =
			SCRPD_MEM_IS_PRIM_DEST | SCRPD_MSC0_IS_ENABLED;
	else
		msc->thdev->output.scratchpad =
			SCRPD_MEM_IS_PRIM_DEST | SCRPD_MSC1_IS_ENABLED;
...
struct msc_win_to_kernel_struct {
	char *buf;
	unsigned long offset;
};

static unsigned long msc_win_to_kernel(void *data, void *src, size_t len)
{
	struct msc_win_to_kernel_struct *k = data;

	memcpy(k->buf + k->offset, src, len);
	k->offset += len;

	return 0;
}

static unsigned long msc_win_to_dummy(void *data, void *src, size_t len)
{
	return 0;
}

static unsigned long msc_total_data_sz(struct msc *msc)
{
	struct msc_window *win;
	unsigned int blk;
	unsigned long sz = 0;
...
static int intel_th_msc_pstore_extra_dumper(char *pstore_buf, size_t len,
					    size_t *read, void *priv)
{
	...
	struct msc *msc = priv;
	struct msc_iter iter;
	struct msc_win_to_kernel_struct k = {
		.buf = pstore_buf,
		.offset = 0,
	};
	...
	size = msc_total_data_sz(msc);

	if (size > len) {
		/* move the iterator to the beginning of the data to dump */
		size_t offset = size - len;

		ret = msc_buffer_iterate(&iter, offset, &k, msc_win_to_dummy);
		if (ret != offset)
			goto out;
	}
	/* dump as much data as possible */
	ret = msc_buffer_iterate(&iter, len, &k, msc_win_to_kernel);
...
static int intel_th_msc_probe(struct intel_th_device *thdev)
{
	...
	msc->pstore_dumper.get_data = intel_th_msc_pstore_extra_dumper;
	msc->pstore_dumper.priv = msc;
	msc->pstore_dumper.name =
		devm_kasprintf(dev, GFP_KERNEL, "%s_dumper", dev->kobj.name);
	if (!msc->pstore_dumper.name)
		return -ENOMEM;
