...
write_to_file(dhd_pub_t *dhd, uint8 *buf, int size)
{
	...
	/* open file to write */
	fp = filp_open(memdump_path, file_mode, 0660);
	if (IS_ERR(fp)) {
		ret = PTR_ERR(fp);
...
do_dhd_log_dump(dhd_pub_t *dhdp)
{
	...
	fp = filp_open(dump_path, file_mode, 0660);
	if (IS_ERR(fp)) {
		ret = PTR_ERR(fp);
	...
	__vfs_write(fp, common_info, strlen(common_info), &pos);
	if (dhdp->dld_buf.wraparound) {
		//fp->f_op->write(fp, dhdp->dld_buf.buffer, DHD_LOG_DUMP_BUFFER_SIZE, &pos);
		__vfs_write(fp, dhdp->dld_buf.buffer, DHD_LOG_DUMP_BUFFER_SIZE, &pos);
	} else {
		//fp->f_op->write(fp, dhdp->dld_buf.buffer,(int)(dhdp->dld_buf.present - dhdp->dld_buf.front), &pos);
		__vfs_write(fp, dhdp->dld_buf.buffer,(int)(dhdp->dld_buf.present - dhdp->dld_buf.front), &pos);
	}
...
dhd_write_file(const char *filepath, char *buf, int buf_len)
{
	...
	if (IS_ERR(fp)) {
		...
		if (fp->f_mode & FMODE_WRITE) {
			...
			ret = __vfs_write(fp, buf, buf_len, &fp->f_pos);
			if (ret < 0) {
